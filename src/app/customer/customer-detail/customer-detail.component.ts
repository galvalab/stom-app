...
export class CustomerDetailComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.actRouter.paramMap.subscribe(params => {
      ...
      // Get Customer Detail
      this.getCustomerDetail(groupid, customerid, agentid, params.get('custcommand'));
  ...
  getCustomerDetail(groupid: string, customerid: string, agentid: string, custcommand: string) {
    const custdetail = this.firestore
      .collection('sto-activity').doc(groupid)
      .collection('customer').doc(customerid).snapshotChanges();

    custdetail.subscribe(customer => {
      // Clear array first
      this.customer = [];
      this.customer.push(customer.payload.get('cust-id'));
      this.customer.push(customer.payload.get('name'));
      this.customer.push(customer.payload.get('address'));
      this.customer.push(customer.payload.get('pic-name'));
      this.customer.push(customer.payload.get('pic-contact'));

      this.urlpath.setLoadingAnimation(false);
    });

    // Set the customer status as on-progress
    this.firestore
      .collection('sto-activity').doc(groupid)
      .collection('customer').doc(customerid).update({
        'on-progress': true
      });
  }
  ...
  openFinishDialog(finishRoute: string, cancelRoute: string, docRefPath: string): void {
  ...
  openDeleteDialog(deleteRoute: string, cancelRoute: string, docRefPath: string): void {
    this.dialog.open(DialogDeleteCustomerComponent, {
      ...
      .afterClosed().subscribe(result => {
        if (typeof (result) === 'undefined') {
          ...
          this.router.navigateByUrl(deleteRoute)
            .then(() => {
